...
double compare2(Feature *feature){
    QString &type = feature->GetFieldValue("type");
    if(type == "primary") return 1;
    return 0.05;
}

bool compare3(Feature * feature){
    QString &type = feature->GetFieldValue("type");
    if(type == "primary") return true;
    return false;
}

bool compare4(Feature * feature){
    QString &type = feature->GetFieldValue("type");
    if(type == "primary") return true;
    return false;
}

bool compare5(QPainter *painter, MapTranslator *renderer, Feature *feature, LabelScheme *lscheme){
    ...
    newLayerCount(1)
{
    ...
    scheme21->SetFilterFunction(compare1);
    ...
    layer5->SetMaxZoomFilter(compare2);
    ...
    scheme51->SetFilterFunction(compare3);
    ...
    labelScheme51->SetFilterFunction(compare4);
    labelScheme51->setPaintFunction(compare5);
...
void mapviewcontroller::keyPressEvent(QKeyEvent *event)
{
    if(event->key() == Qt::Key_Return && (mf->getState() & MapFrame::MapStateEdit)) {
        LayerListItem *item = static_cast<LayerListItem *>(mv->layerList->currentItem());
...
Layer *mapviewcontroller::addLayer(const QString &fileName, ShapeMapReader *smr, Projection *proj,QSimpleSpatial::ShapeTypes shapeType)
{
    Layer *layer;
    ...
    LayerListItem *item = new LayerListItem(mv->layerList);
...
void mapviewcontroller::paint(QPainter &painter)
{
...
void mapviewcontroller::mapClicked(QMouseEvent *event)
{
...
void mapviewcontroller::mapReleased(QMouseEvent *event)
{
    Q_UNUSED(event)
...
void mapviewcontroller::addPointLayer()
{
...
void mapviewcontroller::addPolylineLayer()
{
...
void mapviewcontroller::addPolygonLayer()
{
...
void mapviewcontroller::addPointTriggered(bool checked)
{
    if(checked)
...
void mapviewcontroller::saveLayer()
{

}

void mapviewcontroller::zoom()
{
    mf->GetTranslator()->ZoomTo(1.0);
    mf->updateMap();
}

void mapviewcontroller::zoomIn()
{
    mf->GetTranslator()->ZoomIn();
    mf->updateMap();
}

void mapviewcontroller::zoomOut()
{
    mf->GetTranslator()->ZoomOut();
    mf->updateMap();
}

void mapviewcontroller::layerListChanged(QListWidgetItem *item)
{
...
void mapviewcontroller::layerListSelected()
{
    if(mv->layerList->currentItem())
    {
        ui->actionAddPoint->setEnabled(true);
    }
}

void mapviewcontroller::shapeInfo()
{
    if(!ui->actionShapeInfo->isChecked()) {
        mf->setState(MapFrame::MapStateMove);
